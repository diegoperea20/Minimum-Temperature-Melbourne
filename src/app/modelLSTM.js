...
const ModelComponent = () => {
  ...
  const [lastTempSequence, setLastTempSequence] = useState(null);

  useEffect(() => {
    async function loadModel() {
      setIsLoading(true);
      const modelUrl = `${window.location.origin}/model/modelLSTM/model.json`;
  ...
  const fetchData = async () => {
    ...
    // Set last 10 temperature sequence
    const last10Temps = normalizedTemp.values.slice(-10).map(v => [v]);
    setLastTempSequence([last10Temps]);
    ...
    if (model) {
      ...
      for (let dateStr of datesToPredict) {
        ...
        const tempInputTensor = tf.tensor3d(tempSequence);
        const predictionTensor = model.predict([tempInputTensor, scaledInput]);
        const prediction = scalerTemp.inverseTransform(predictionTensor).dataSync()[0];
        predictions.push(prediction);
        
        // Update temperature sequence for next prediction
        tempSequence[0] = [...tempSequence[0].slice(1), [predictionTensor.dataSync()[0]]];
      }
  ...
  const predictTemperature = async (dateStr) => {
    if (!model || !scalerFeatures || !scalerTemp || !lastTempSequence) {
      console.error('Modelo, escaladores o secuencia de temperatura no cargados');
      return null;
    }
    ...
    const inputTensor = tf.tensor2d([[day, month, year]]);
    const scaledInput = scalerFeatures.transform(inputTensor);
    const tempInputTensor = tf.tensor3d(lastTempSequence);
    const predictionTensor = model.predict([tempInputTensor, scaledInput]);
    const prediction = scalerTemp.inverseTransform(predictionTensor).dataSync()[0];
